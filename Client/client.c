...
int main(int argc, char *argv[])
{
    ...
    if (c == 'Y')
    {
        ...
        {
            nodeSSL = SSL_new(ctx2); //Creating new SSL connectin state
            SSL_set_fd(nodeSSL, client); //set client as the IO facility for encryption
        }
        
        if (SSL_accept(nodeSSL) <= 0)
		{
		...
		else
		{
            ...
            InsertClient(nodeSSL, connectedClient, client);
    ...
    ssl = SSL_new(ctx1);      /* create new SSL connection state */
    ...
    else
    {
        ...
        {
            if (pthread_create(&clientCom, NULL, ClientCommunicate, (void *)ssl) != 0)
            {
        ...
        else
        {
            ...
            if (pthread_create(&threadID2, NULL, NodeListenToServer, (void *)nodeSSL) != 0)
            {
